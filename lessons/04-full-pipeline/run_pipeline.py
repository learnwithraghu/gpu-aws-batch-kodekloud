"""
Lesson 04 — run_pipeline.py
Runs LOCALLY. Submits the images → captions → verification pipeline as two
chained Batch jobs.

Job 1: generate_captions   (from lesson 03)
Job 2: verify_captions     (runs only after job 1 SUCCEEDS)
...
def submit_pipeline(image_prefix: str, batch_stem: str) -> tuple[str, str]:
    ...
    # ── Job 2: verify captions — only starts after Job 1 SUCCEEDS ───────────
    job2 = batch.submit_job(
        jobName      = f"pipeline-verify-{batch_stem}",
        jobQueue     = JOB_QUEUE,
        jobDefinition= JOB_DEFINITION,
        dependsOn    = [{"jobId": job1_id, "type": "N_TO_N"}],   # ← the key line
        containerOverrides={
            "command": ["python", "/app/lessons/04-full-pipeline/verify_captions.py"],
            "environment": [
                {"name": "S3_BUCKET",    "value": S3_BUCKET},
                {"name": "IMAGE_PREFIX", "value": image_prefix},
            ],
...
if __name__ == "__main__":
    ...
    print(f"Job 2 (verify) : {'✅' if ok2 else '❌'}  {state2}")
