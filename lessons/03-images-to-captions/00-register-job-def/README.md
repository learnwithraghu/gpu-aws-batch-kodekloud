...
pushed to ECR in lesson 00 to the GPU queue: **4 vCPU · 16 GiB · 1 GPU**
(a full g4dn.xlarge), with the course S3 buckets as default environment
variables.

Idempotent — run it again and it reuses the existing registration; if the
`ECR_IMAGE_URI` in `.env` has changed, it registers a new revision instead.
