...
# 1. Find every captions/<stem>/captions.csv in the CSV bucket
stems = set()
paginator = s3.get_paginator("list_objects_v2")
for page in paginator.paginate(Bucket=S3_CSV_BUCKET, Prefix="captions/"):
...
if not stems:
    print(f"No caption files yet in s3://{S3_CSV_BUCKET}/captions/")
else:
    print(f"Caption files in s3://{S3_CSV_BUCKET}/captions/:\n")
    total = 0
    for stem in sorted(stems):
        key = f"captions/{stem}/captions.csv"
        body = s3.get_object(Bucket=S3_CSV_BUCKET, Key=key)["Body"].read()
